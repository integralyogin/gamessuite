...
const bitCraftGame = {
    ...
    render: function() {
        ...
        document.getElementById('bitcraft-exit-btn').onclick = () => this.successCallback({ message: 'Exited bitCraft v0.03 (FIXED)' });
    ...
    selectEntitiesInBox: function() {
        ...
        if (box.w < GameConfig.input.dragThreshold && box.h < GameConfig.input.dragThreshold) {
            ...
            if (clickedEntity && (clickedEntity.type === 'player_unit' || clickedEntity.type === 'lightningTurret' || clickedEntity.type === 'workbench')) {
    ...
    updateSideMenu: function() {
        ...
        if (perfDiv && GameConfig.settings.debugMode) {
            perfDiv.innerHTML = `FPS: ${this.performance.currentFPS}<br>Entities: ${this.gameObjects.length}<br>Effects: ${MagicSystem.effects.length}`;
        }
